...
const Editor = () => {
  ...
  const [suggestion, _setSuggestion] = useState("print(output)");
  const suggestionRef = React.useRef(suggestion);
  const [value, setValue] = useState("");

  function setSuggestion(value) {
    suggestionRef.current = value; // Updates the ref
    _setSuggestion(value);
  }
  ...
  const makeRequest = async () => {
    ...
    await fetch("http://localhost:2000/complete", {
      ...
      .then((res) => {
        res.json().then((data) => {
          console.log("Encrypted Response:", encrypt(data.completed_code));
          setSuggestion(data.completed_code);
        });
      })
  ...
  const addSuggestion = (edit) => {
    if (suggestionRef.current === "loading...") {
      return;
    }
    //console.log(edit.state.selection.main);
    let transaction = edit.state.update({
      changes: {
        from: edit.state.selection.main.from,
        insert: suggestionRef.current,
      },
    });
